...
/// ```
pub async fn get_recommend_list(ps: i32, fresh_idx: i32) -> Result<Vec<RcmdVideoInfo>, ApiError> {
    ...
    if !response.status().is_success() {
        match response.status().as_u16() {
            ...
            _ => return Err(ApiError::ApiError {
    ...
    if rcmd_response.code != 0 {
        return Err(ApiError::ApiError {
            code: rcmd_response.code,
            message: rcmd_response.message
        });
    }
