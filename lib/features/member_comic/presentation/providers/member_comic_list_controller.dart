...
class MemberComicListState {
  MemberComicListState({
...
/// Controller for member comic list
class MemberComicListController extends ChangeNotifier {
  MemberComicListController({
    required this.mid,
    required FetchMemberComicsUseCase fetchComics,
  }) : _fetchComics = fetchComics {
    queryData(isRefresh: true);
  }

  final int mid;
  final FetchMemberComicsUseCase _fetchComics;

  MemberComicListState _state = MemberComicListState();

  MemberComicListState get state => _state;
  ...
  Future<void> queryData({bool isRefresh = true}) async {
    ...
    final result = await _fetchComics(mid: mid, page: page);
    ...
    if (result is Loading) {
      ...
      if (dataList.isEmpty) {
        ...
        if (isRefresh) {
          ...
          if (currentState is Success<List<MemberComicItemEntity>?>) {
