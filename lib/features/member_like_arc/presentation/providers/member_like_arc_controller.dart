...
import 'package:PiliPlus/features/member_like_arc/presentation/providers/member_like_arc_list_provider.dart';

part 'member_like_arc_controller.g.dart';

/// State for member like arc list
class MemberLikeArcState {
  MemberLikeArcState({
  ...
  MemberLikeArcState copyWith({
    LoadingState<List<MemberLikeArcItemEntity>?>? listState,
    bool? isLoading,
    bool? isEnd,
    int? currentPage,
  }) {
    return MemberLikeArcState(
...
@riverpod
class MemberLikeArcController extends _$MemberLikeArcController {
  @override
  ...
  Future<void> queryData(int mid, {bool isRefresh = true}) async {
    ...
    final fetchLikeArcs = ref.read(fetchMemberLikeArcsUseCaseProvider);
    final result = await fetchLikeArcs(mid: mid, page: page);
    ...
    if (result is Loading) {
      ...
      if (dataList.isEmpty) {
        ...
        if (isRefresh) {
          ...
          if (currentStateList is Success<List<MemberLikeArcItemEntity>?>) {
