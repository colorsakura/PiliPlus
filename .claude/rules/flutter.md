...
* **Riverpod:** Use `flutter_riverpod` as the primary state management solution.
  Riverpod is a reactive caching and data-binding framework that handles errors/loading
  states, supports advanced scenarios, and separates logic from the UI. It provides
  compile-time safety, excellent testability, and works well with Clean Architecture.
...
* **Installation:** Add dependencies to your project.
  ...
  # Run code generator (if using @riverpod annotations)
...
* **ProviderScope:** Wrap your `MaterialApp` with `ProviderScope` at the root level
  to enable Riverpod throughout the app. This is the Flutter equivalent of
  `ProviderContainer` and manages provider state for the entire app.
...
* **ConsumerWidget:** Use `ConsumerWidget` instead of `StatelessWidget` to access
  providers via `WidgetRef`. These widgets bridge the gap between the widget tree
  and the provider tree.
...
* **ConsumerStatefulWidget:** Use `ConsumerStatefulWidget` when you need `State`
  lifecycle management alongside provider access. This is necessary when managing
  local state like `TextEditingController`.
...
* **Ref Methods:** Understanding when to use each ref method is crucial.

  - `ref.watch()`: Use in build method for reactive UI. Subscribes to provider and
    rebuilds widget when state changes.
  - `ref.read()`: Use in callbacks, NOT in build. Gets current value without
    listening. Do NOT use as a means to ignore changes.
  - `ref.listen()`: Use for side effects (snackbars, navigation, dialogs).
    Reacts to state changes without rebuilding.
  - `ref.invalidate()`: Reset provider state, forcing rebuild on next access.
  - `ref.refresh()`: Invalidate and immediately read new value.
  - `ref.select()`: Watch only a specific part of state, rebuilds only when that
    part changes.
  ...
  class RefExampleWidget extends ConsumerWidget {
    @override
    Widget build(BuildContext context, WidgetRef ref) {
      // ✅ ref.watch - Use in build method for reactive UI
      final count = ref.watch(counterProvider);

      // ✅ ref.listen - Use for side effects
      ...
      // ✅ ref.select - Watch only specific part
...
* **Provider Types:** Choose the appropriate provider for your use case.

  - **Provider**: For read-only, synchronous values
  - **NotifierProvider**: For mutable synchronous state with complex logic
  - **FutureProvider**: For asynchronous values that complete once
  - **StreamProvider**: For streams of values
  - **AsyncNotifierProvider**: For mutable asynchronous state

* **Creating Providers with Code Generation (Recommended):**

  Use the `@riverpod` annotation for type-safe, auto-completed providers.
  ...
  // Basic provider - read-only synchronous value
  @riverpod
  ...
  // FutureProvider - asynchronous value
  @riverpod
  ...
  // AsyncNotifierProvider - mutable asynchronous state
  @riverpod
  class TodoList extends _$TodoList {
    ...
    Future<void> addTodo(String title) async {
      final newTodo = await postTodoToApi(title);
...
* **Creating Providers without Code Generation:**
  ...
  // FutureProvider
  final activityProvider = FutureProvider<String>((ref) async {
...
* **Provider Families:** Create parameterized providers using records for multiple
  parameters.
...
* **Async State Handling:** Use pattern matching with `AsyncValue` for loading,
...
* **Dependency Injection:** Providers can depend on other providers. Riverpod
  automatically manages dependency lifecycle and updates.
...
* **Dark/Light Mode and Theme Toggle:** Implement support for both light and
  ...
  ```dart
  // Define theme mode provider with code generation
  @riverpod
  ...
  // Usage in settings widget
  ElevatedButton(
    onPressed: () => ref.read(themeModeNotifierProvider.notifier).setDarkMode(),
